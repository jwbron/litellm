...
def test_translate_streaming_openai_chunk_to_anthropic_thinking_content_block():
    ...
    # The block start is always empty (Anthropic protocol): the chunk's
    # thinking content flows via the re-queued thinking_delta instead, so
    # clients that concatenate block_start body + deltas don't see it twice.
    assert content_block_start == {
        "type": "thinking",
        "thinking": "",
        "signature": "",
...
def test_translate_streaming_openai_chunk_to_anthropic_thinking_signature_block():
    ...
    assert block_type == "thinking"
    # Empty block start (Anthropic protocol): the signature flows via the
    # re-queued signature_delta, not the block start body.
    assert content_block_start == {
        "type": "thinking",
        "thinking": "",
        "signature": "",
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Claude models are covered by is_anthropic_claude_model, not this gate.
        ("anthropic/claude-sonnet-4", False),
    ],
)
def test_supports_cache_control_passthrough(model, expected):
    """
    cache_control passthrough in the Anthropic->OpenAI adapter is gated on
    is_anthropic_claude_model OR supports_cache_control_passthrough. Alibaba
    (Qwen) upstreams require explicit cache_control breakpoints on OpenRouter,
    so qwen must be recognized by the passthrough gate; DeepSeek must not (its
    caching is automatic and ignores cache_control). The claude gate itself
    must stay claude-only, because it also controls thinking-param passthrough.
    """
    assert LiteLLMAnthropicMessagesAdapter.supports_cache_control_passthrough(model) is expected


def test_is_anthropic_claude_model_stays_claude_only():
    """The thinking-param gate must not be widened to non-Claude models."""
    assert LiteLLMAnthropicMessagesAdapter.is_anthropic_claude_model("anthropic/claude-sonnet-4") is True
    assert LiteLLMAnthropicMessagesAdapter.is_anthropic_claude_model("openrouter/qwen/qwen3-max") is False
...
def test_add_system_message_filters_billing_header_block():
    ...
    # cache_control survives for qwen (gated by supports_cache_control_passthrough)
