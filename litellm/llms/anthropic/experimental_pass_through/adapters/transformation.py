...
class LiteLLMAnthropicMessagesAdapter:
    ...
    def _add_cache_control_if_applicable(
        ...
        if (
            ...
            and (
                ...
                or self.supports_cache_control_passthrough(model)
            )
        ):
    ...
    def supports_cache_control_passthrough(model: str) -> bool:
        """
        Check if the model's upstream honors explicit ``cache_control``
        content blocks, so the blocks should survive the Anthropic->OpenAI
        translation even though the model is not a Claude model.

        Qwen via OpenRouter/Alibaba requires explicit ``cache_control``
        blocks for prompt caching (see
        https://openrouter.ai/docs/features/prompt-caching); stripping them
        makes every turn pay full input rate. This is deliberately NOT part
        of :meth:`is_anthropic_claude_model`: that gate also controls
        ``thinking``-param passthrough, and qwen needs thinking converted to
        ``reasoning_effort``, not passed through.
        """
            ...
            if (
                ...
                if len(thinking_blocks) > 0:
                    ...
                    if thinking_block["type"] == "thinking":
                        # Anthropic protocol: a content_block_start always
                        # carries an EMPTY body; content flows via deltas
                        # (matches the text / reasoning_content branches, and
                        # the streaming iterator's step-3 re-queue of the
                        # trigger chunk's delta). Embedding the first chunk's
                        # thinking/signature here duplicates it for clients
                        # that concatenate block_start body + deltas: the same
                        # content is also emitted as the re-queued
                        # thinking_delta / the terminal signature_delta.
